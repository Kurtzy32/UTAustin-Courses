...
class UpdateUserForm(forms.Form):
    ...
    def clean(self):
        ...
        if cleaned_data['update_user_secrecy'] < UserXtraAuth.objects.get(username=cleaned_data['update_user_select']).secrecy:
            raise forms.ValidationError("Cannot reduce") 
        if cleaned_data['update_user_secrecy'] > 0 and cleaned_data['update_user_token'] == "":
            raise forms.ValidationError("Need a token")
        return cleaned_data
...
class CreateNewsForm(forms.Form):
    ...
    def clean(self):
        ...
        if self.user_secrecy > cleaned_data['new_news_secrecy']:
            raise forms.ValidationError("Need a higher secrecy level")
...
class UpdateNewsForm(forms.Form):
    ...
    def clean(self, user_secrecy=0):
        ...
        cleaned_data = super().clean()
        if cleaned_data["update_news_secrecy"] and cleaned_data["update_news_secrecy"] < self.sec_inst:
            raise forms.ValidationError("Need a higher secrecy level")
